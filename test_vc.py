...
def sharp_break(F_norm, v, vm, vc, p=2.25):
...
plt.tight_layout()
plt.show()
#plt.savefig('smooth_vs_sharp_spectra.pdf')
...
for v_c in vc_values:
	...
	plt.plot(v_values, flux_values_sharp, color='orange', linewidth=2)
	...
	plt.yscale('log')
	plt.xlabel(r'Frequency ($\nu$) [Hz]', fontsize=12)
	plt.ylabel('Flux (F)', fontsize=12)
	plt.grid(True, which="both", ls="-", alpha=0.2)
	#plt.legend(fontsize=10)
	#plt.show()
	#plt.savefig(f'{v_c:.3e}_sharp_vs_smooth.pdf')
	plt.savefig('poster_spectrum.pdf')
...
for i, v_c in enumerate(vc_values):
		flux_smooth_opt = spectrum(F_0, (4.61E14), v_m, v_c)
		flux_smooth_x = spectrum(F_0, (2.418E17), v_m, v_c,)
		flux_sharp_opt = sharp_break(F_0, (4.61E14), v_m, v_c, p)
		flux_sharp_x = sharp_break(F_0, (2.418E17), v_m, v_c, p)
		B_ox_smooth.append((np.log10(flux_smooth_opt/flux_smooth_x))/(np.log10(4.61E14/2.418E17)))
		B_ox_sharp.append((np.log10(flux_sharp_opt/flux_sharp_x))/(np.log10(4.61E14/2.418E17)))
		#print(B_ox_smooth, B_ox_sharp, v_c)
'''
...
#plt.show()
plt.savefig('spectral_indices.pdf')
'''
